...
void update_air_without_turn(struct MarioState *m) {
    ...
    if (!check_horizontal_wind(m)) {
        ...
        if (m->input & INPUT_NONZERO_ANALOG) {
            ...
            sidewaysSpeed = intendedMag * sins(intendedDYaw) * 1.5f;
        }
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->actionState == 1) {
        ...
        mario_set_forward_vel(m, 0.0f);
        set_mario_animation(m, m->actionState == 0 ? MARIO_ANIM_START_GROUND_POUND
                                                 : MARIO_ANIM_TRIPLE_JUMP_GROUND_POUND);
...
s32 act_shot_from_cannon(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_NONE:
            ...
            m->faceAngle[0] = atan2s(m->forwardVel, m->vel[0]);
            m->marioObj->header.gfx.angle[0] = -m->faceAngle[0];
            break;
    ...
    if ((m->flags & MARIO_NORMAL_CAP) && m->vel[1] < 0.0f) {
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_BACKFLIP:             cancel = act_jump(m);             break;
        ...
        case ACT_JUMP_KICK:            cancel = act_dive(m);            break;
